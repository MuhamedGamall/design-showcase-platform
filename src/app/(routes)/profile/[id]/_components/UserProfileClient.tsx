...
import { TooltipHover } from "./TooltibHover";
...
const UserProfileClientContent = async ({ params }: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [projectRes, sessionRes, followingRes] = await Promise.all([
          getUserProjects({ userId: params?.id }),
          getCurrentSession(),
          getFollowing(),
        ]);

        setProjects(projectRes.projects);
        setUser(projectRes.user);
        setSession(sessionRes.session);
        setFollowingData(followingRes.followingData);
      } catch (error) {
      ...
      <Container>
        <section className="flexCenter flex-col  w-full mx-auto paddings">
          ...
          <section className="flexStart flex-col lg:mt-28 mt-16 w-full">
            ...
            <ProjectsContent data={projects} loading={loading} />
          </section>
        </section>
      </Container>
      {session?.user?._id === params?.id && (
...
export default UserProfileClientContent;
